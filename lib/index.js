...
async function walkSearchResults(params) {

  const results = [];

  let $ = await carSearch(params);

  while($('.listingsNextPage a')) {
    ...
    for (const node of listings) {
      ...
      results.push(await getCarDetails(url));
    }

    $ = cheerio.load(await request($('.listingsNextPage a').attr('href')));
  }
...
async function scrape() {
  ...
  const data = await walkSearchResults({
    radius: 50,
    zip:    20001,
    year:   2015,
    make:   'BMW',
