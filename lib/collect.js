...
async function collect() {
  ...
  const dealerCache = {};
  async function getDealer(record) {
    const id = record.dealer_dealerId;
    if (dealerCache[id]) return dealerCache[id];

    const dealer = await api.get(`dealer/v2/dealers/${id}`);

    // delete uneeded info
    delete dealer.operations
    const phone = _.get(dealer, 'contactInfo.phone');
    const website = _.get(dealer, 'contactInfo.website');
    delete dealer.contactInfo;
    delete dealer.states;
    delete dealer.type;

    // cache dealer for fast lookup again
    return (dealerCache[id] = { phone, website, ...dealer });
  }
      ...
      try {
        ...
        do {
          ...
          for (const record of records) {
            const dealer = await getDealer(record);
            record.dealer = dealer;
            csvStream.write(flatten(record));
          }
